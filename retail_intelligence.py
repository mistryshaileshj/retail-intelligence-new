...
# Summary cards: 7 metrics available, up to 5 shown (chosen & ordered in the sidebar).
SUMMARY_OPTIONS = ["Net Sales", "Gross Sales", "Units Sold", "Transactions",
                   "Avg Sell Price", "Return Rate", "Gross Margin"]
DEFAULT_SUMMARY = ["Net Sales", "Units Sold", "Avg Sell Price", "Return Rate", "Gross Margin"]
MAX_SUMMARY = 5
...
with st.sidebar:
    ...
    st.markdown('<div class="side-title" style="margin-top:16px;">📇 Summary cards</div>',
                unsafe_allow_html=True)
    summary_sel = st.multiselect("Pick up to 5 (order = selection)", SUMMARY_OPTIONS,
                                 default=DEFAULT_SUMMARY, max_selections=MAX_SUMMARY,
                                 key="summary_cards")
    st.caption("Max 5 · no duplicates · remove & re-add to reorder")
...
METRICS = {
    "Net Sales":      (f"₹{net:,.0f}",     f"Gross ₹{gross:,.0f}",                   "up",      "💰"),
    "Gross Sales":    (f"₹{gross:,.0f}",   "before returns",                         "up",      "🧾"),
    "Units Sold":     (f"{units:,.0f}",    f"{txns} transactions",                   "neutral", "📦"),
    "Transactions":   (f"{txns:,.0f}",     "unique bills",                           "neutral", "🧮"),
    "Avg Sell Price": (f"₹{avg_pr:,.0f}",  f"Disc: {avg_disc:.1f}%",                 "neutral", "🏷️"),
    "Return Rate":    (f"{rtn_rate:.1f}%", f"₹{rtn_amt:,.0f} | {rtn_qty:.0f} units", "down" if rtn_rate > 10 else "up", "↩️"),
    "Gross Margin":   (f"{margin:.1f}%",   f"₹{tot_disc:,.0f} discounted",           "up" if margin > 30 else "neutral", "📊"),
}
sel_cards = summary_sel[:MAX_SUMMARY]
if sel_cards:
    mcols = st.columns(len(sel_cards))
    for i, lbl in enumerate(sel_cards):
        val, delta, direction, icon = METRICS[lbl]
        kpi(mcols[i], lbl, val, delta, direction, f"k{i + 1}", icon)
...
CHART_TYPES = ["Bar", "H-Bar", "Line"]
CHART_ICONS = {"Bar": "📊", "H-Bar": "📊", "Line": "📈"}
CHART_ICON_MAP = {
    "Bar":   ":material/bar_chart:",
    "H-Bar": ":material/align_horizontal_left:",
    "Line":  ":material/show_chart:"
}
